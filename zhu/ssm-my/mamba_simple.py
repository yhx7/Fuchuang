        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            if self.bimamba_type == "v6":
                ...
                out = mamba_inner_fn_no_out_proj(
                    ...
                    # delta_softplus=True,
                    self.dt_proj.bias.float(),   # delta_bias
                    None,                        # B_proj_bias
                    None,                        # C_proj_bias
                    True,                        # delta_softplus
                    1                            # checkpoint_lvl ✅ 必须显式传
                )
                ...
                xz_e = rearrange(xz, "b d l -> b l d")
                xz_e = local_scan(xz_e, w=local_size, H=self.input_h, W=self.input_w)
                x_e, z_e = xz_e.chunk(2, dim=1)
                extra_emb_e = rearrange(extra_emb, "b d l -> b l d")
                extra_e = local_scan(extra_emb_e, w=local_size, H=self.input_h, W=self.input_w)

                xz_f = rearrange(xz, "b d l -> b l d")
                xz_f = local_scan(xz_f, w=local_size, H=self.input_h, W=self.input_w, flip=True)
                x_f, z_f = xz_f.chunk(2, dim=1)
                extra_emb_f = rearrange(extra_emb, "b d l -> b l d")
                extra_f = local_scan(extra_emb_f, w=local_size, H=self.input_h, W=self.input_w, flip=True)
                ...
                y_e = local_reverse(y_e, w=local_size, H=self.input_h, W=self.input_w)
                y_f = local_reverse(y_f, w=local_size, H=self.input_h, W=self.input_w, flip=True)


                if not self.if_devide_out:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f, "b d l -> b l d")),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f, "b d l -> b l d"),
                                       self.out_proj.weight, self.out_proj.bias)
                else:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f, "b d l -> b l d") / 4),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(y + y_b.flip([-1]) + y_c + y_d + y_e + y_f, "b d l -> b l d") / 4,
                                       self.out_proj.weight, self.out_proj.bias)
